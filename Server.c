...
#define NUMBER_CLIENTS 10 /* Number of usernames and passwords */
...
int thr_id[NUM_HANDLER_THREADS][3];  /* col 1: thread IDs. col 2: 1/0 (serving client)/(waiting for client) */     
pthread_t parent_thread_id; /* parent thread ID  */
pthread_t accepter_thread_id;
...
void SendNumberTo_Client(int new_fd, int* number);

int SelectRandomNumber();
char* concat(const char *s1, const char *s2);
void signalhandler(int signum, siginfo_t *si, void* unused);
...
while (1) {
	...
	a_request = get_request(&request_mutex);
        if (a_request) { /* got a request - handle it and free it */
			...
			served_clients--; //decrement number of clients playing
...
for(int ii = 0; ii < NUMBER_CLIENTS; ii++){
	...
	thread_state_ptr = (threadState_t*)malloc(sizeof(ThreadState));
		...
		thr_id[(*thread_state_ptr).current_player][3] = *new_fd; // thread is serving a client
...
RecvNumberFrom_Client(new_fd, choice);
...
if((choice >= 1) && (choice <=3)){
	SendNumberTo_Client(new_fd, &answer);
	return true;
}else{
	answer = 0;
	SendNumberTo_Client(new_fd, &answer);
...
network_byte_order_short = htons(*number);
...
/* Check games played >0 for client and send back respose */
if(games_played <= 0){ 
...
read_count++;
if(read_count == 1){
	sem_wait(&rw); // 1st reader blocks writer
}
pthread_mutex_unlock(&reader);

/* Read Now */

/* Algorithm fo sending data:
...
/* Go through leaderboard struct and keep indexes of players who have played a game */
...
/* Sort player_index array so that it reflects game rules, using bubble sort */
...
SendNumberTo_Client(new_fd, &players_played);


/* Loop through players who have played, and send their statistics */
for(int k = 0; k < players_played; k++){
	...
	SendNumberTo_Client(new_fd, &Leaderboard.gamesWon[player_index[k]]);

	// Send to client their number of games played
	SendNumberTo_Client(new_fd, &Leaderboard.gamesPlayed[player_index[k]]);
}
...
read_count--;
if(read_count == 0){
	sem_post(&rw); // last reader unblocks writer
}
pthread_mutex_unlock(&reader);
...
printf("Im at PlayGame. "); // For debuggin purposes
fflush(stdout); /* make sure this gets printed to screen */

/* Select a random index into hangman words, and calculate guesses left */
...
SendNumberTo_Client(new_fd, &guess_left);
printf("guess left = %d\n", guess_left);
...
while((guess_left > 0) && (letters_left_to_guess > 0) && keeprunning){
	...
	// send have won to the client
		toSend = 1;
		SendNumberTo_Client(new_fd, &toSend);
		break;
	}else{
		toSend = 0;
		SendNumberTo_Client(new_fd, &toSend);
...
while(keeprunning){
	...
	if(choice == QUIT && keeprunning){ 
		printf("Client quit: closing socket and exiting\n");
...
/* Game vairiables*/
int games_won = 0; int games_played = 0; int choice = 0; int i = 0; int sig;
bool validInput = false;

/* Socket Variables */
int  new_fd, portNumber;  /* listen on sock_fd, new connection on new_fd */
struct sockaddr_in my_addr;    /* my address information */
struct sockaddr_in their_addr; /* connector's address information */
socklen_t sin_size = sizeof(struct sockaddr_in);
socketInfo* socketInfo_ptr;
...
/* handling for ctl+c user input */	
sigemptyset(&act.sa_mask);
act.sa_sigaction = &signalhandler;

// Block SIGINT signals on threads
sigaddset(&act.sa_mask, SIGINT);
if(pthread_sigmask(SIG_BLOCK, &act.sa_mask, NULL) == -1){
	perror("sigmask:");
}

// handle differnet signals
...
// Create a thread to handle accepting new client
pthread_create(&accepter_thread_id, NULL, handle_client_requests, NULL); 

/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 Now lets wait for a ctl+c signal and then clean up
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
sigemptyset(&waitset);
sigaddset(&waitset, SIGUSR1);
sigaddset(&waitset, SIGINT);
pthread_sigmask(SIG_SETMASK, &waitset, NULL); // have the parent thread only wait on an INT

// Wait for a signal - then close other threads and deallocate memory
if(sigwait(&waitset, &sig) != 0){
	...
	printf("\nEXIT signal: Exiting as soon as possible\n");
	...
	// Send kill SIGUSR1 signal to accept handler thread
	pthread_kill(accepter_thread_id, SIGUSR1);

	// Send kill SIGUSR1 signal to all other threads
	for(int jj = 0; jj < NUM_HANDLER_THREADS; jj++){
			pthread_kill(pthr_id[jj], SIGUSR1); // deallocate memory wherever the thread is

			// if thread is serving a client, close its socket
			if(thr_id[jj][1] == 1){ 
				close(thr_id[jj][2]);
			}
	} 
	sleep(1); // wait here for a second until other threads close 
...
switch(signum){
	...
	case SIGUSR1 :
		...
		if(pthread_equal(pthread_self(), accepter_thread_id) != 0){    
			pthread_exit(NULL);
			break;
		}
